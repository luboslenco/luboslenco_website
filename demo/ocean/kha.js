...
var lue_App = function(room,game) {
	kha_Game.call(this,"Game");
	this.room = room;
	this.game = game;
};
...
lue_App.__super__ = kha_Game;
lue_App.prototype = $extend(kha_Game.prototype,{
	init: function() {
		kha_Configuration.setScreen(new kha_LoadingScreen());
		lue_App.w = this.width;
		lue_App.h = this.height;
...
lue_Eg.render = function(g,camera) {
	camera.renderFrame(g,lue_Eg.root,lue_node_Node.lights[0]);
};
...
lue_node_Node.addScene = function(name,parent) {
	...
	while(_g < _g1.length) {
		...
		++_g;
		var node = null;
		if(n.type == "camera_node") node = lue_Eg.addCameraNode(lue_resource_Resource.getCamera(name,n.object_ref)); else if(n.type == "light_node") node = lue_Eg.addLightNode(lue_resource_Resource.getLight(name,n.object_ref)); else if(n.type == "geometry_node") {
			var materials = [];
			var _g2 = 0;
			var _g3 = n.material_refs;
			while(_g2 < _g3.length) {
				var ref = _g3[_g2];
				++_g2;
				materials.push(lue_resource_Resource.getMaterial(name,ref));
			}
			node = lue_Eg.addModelNode(lue_resource_Resource.getModel(name,n.object_ref),materials);
		}
...
var lue_node_CameraNode = function(resource) {
	...
	if(resource.resource.type == "perspective") this.P = lue_math_Mat4.perspective(45,lue_App.w / lue_App.h,resource.resource.near_plane,resource.resource.far_plane); else this.P = lue_math_Mat4.orthogonal(-10,10,-6,6,-resource.resource.far_plane,resource.resource.far_plane,2);
...
lue_node_CameraNode.prototype = $extend(lue_node_Node.prototype,{
	renderFrame: function(g,root,light) {
		this.updateMatrix();
		...
		while(_g < _g1.length) {
			...
			if(stage.command == "set_target") {
				this.setTarget(stage.params[0]);
				this.begin(this.currentRenderTarget);
			} else if(stage.command == "clear_target") this.clearTarget(this.currentRenderTarget); else if(stage.command == "draw_geometry") {
				this.drawGeometry(this.currentRenderTarget,stage.params[0],root,light,this.bindParams);
				this.end(this.currentRenderTarget);
			} else if(stage.command == "bind_target") this.bindTarget(this.currentRenderTarget,stage.params); else if(stage.command == "draw_quad") {
				this.drawQuad(this.currentRenderTarget,stage.params,this.bindParams);
				this.end(this.currentRenderTarget);
			}
		}
	}
	...
	,clearTarget: function(g) {
		g.clear(this.clearColor,1,null);
	}
	,drawGeometry: function(g,context,root,light,bindParams) {
	...
	,bindTarget: function(g,params) {
		this.bindParams = params;
	}
	,drawQuad: function(g,params,bindParams) {
		var matRes = lue_resource_Resource.getMaterial(params[0],params[1]);
		var materialContext = matRes.getContext(params[2]);
		var context = matRes.shader.getContext(params[2]);
		g.setDepthMode(false,kha_graphics4_CompareMode.Always);
		g.setCullMode(kha_graphics4_CullMode.None);
		...
		while(_g1 < _g) {
			...
			if(c.type == "mat4") {
				var m = null;
				if(c.link == "_viewMatrix") m = this.V; else if(c.link == "_projectionMatrix") m = this.P;
				if(m == null) return;
		...
		if(materialContext.textures != null) {
			var _g11 = 0;
			var _g2 = materialContext.textures.length;
			while(_g11 < _g2) {
				var i1 = _g11++;
				g.setTexture(context.textureUnits[i1],materialContext.textures[i1]);
			}
		}
		if(bindParams != null) {
			var _g12 = 0;
			var _g3 = bindParams.length / 2 | 0;
			while(_g12 < _g3) {
				var i2 = _g12++;
				var pos = i2 * 2 + 1;
				var _g31 = 0;
				...
				while(_g31 < _g21) {
					var j = _g31++;
					if(bindParams[pos] == context.resource.texture_units[j].id) g.setTexture(context.textureUnits[j],this.resource.pipeline.renderTargets.get(bindParams[pos - 1]));
				}
			}
		}
	...
	,moveForward: function(f) {
		var v3Move = this.viewMatrixLook();
		v3Move.mult(-f,v3Move);
		this.moveCamera(v3Move);
		return v3Move;
	}
	,moveRight: function(f) {
		var v3Move = this.viewMatrixRight();
		v3Move.mult(-f,v3Move);
		this.moveCamera(v3Move);
		return v3Move;
	}
	,moveUp: function(f) {
		var v3Move = this.viewMatrixUp();
		v3Move.mult(-f,v3Move);
		this.moveCamera(v3Move);
		return v3Move;
	}
...
lue_node_LightNode.prototype = $extend(lue_node_Node.prototype,{
	buildMatrices: function() {
		this.P = lue_math_Mat4.perspective(45,1,5,30);
		this.V = lue_math_Mat4.lookAt(new lue_math_Vec3(0,2,10),new lue_math_Vec3(0,0,0),new lue_math_Vec3(0,0,1));
	}
...
var lue_node_ModelNode = function(resource,materials) {
	...
	if(lue_node_ModelNode.helpMat == null) lue_node_ModelNode.helpMat = new lue_math_Mat4();
...
lue_node_ModelNode.prototype = $extend(lue_node_Node.prototype,{
	...
	,setConstants: function(g,context,camera,light,bindParams) {
		var _g1 = 0;
		var _g = context.resource.constants.length;
		while(_g1 < _g) {
			var i = _g1++;
			var c = context.resource.constants[i];
			this.setConstant(g,camera,light,context.constants[i],c);
		}
		if(bindParams != null) {
			var _g11 = 0;
	...
	,setConstant: function(g,camera,light,location,c) {
		if(c.link == null) return;
		if(c.type == "mat4") {
			var m = null;
			if(c.link == "_modelMatrix") m = this.transform.matrix; else if(c.link == "_viewMatrix") m = camera.V; else if(c.link == "_inverseViewMatrix") {
				...
				lue_node_ModelNode.helpMat.mult(this.transform.matrix);
	...
	,setMaterialConstants: function(g,context,materialContext) {
		var _g1 = 0;
		...
		while(_g1 < _g) {
			...
			var c = context.resource.constants[i];
			this.setMaterialConstant(g,context.constants[i],c,matc);
		}
		if(materialContext.textures != null) {
			var _g11 = 0;
			var _g2 = materialContext.textures.length;
	...
	,setMaterialConstant: function(g,location,c,matc) {
	...
	,render: function(g,context,camera,light,bindParams) {
		...
		g.setVertexBuffer(this.resource.geometry.vertexBuffer);
		this.setConstants(g,shaderContext,camera,light,bindParams);
		var _g11 = 0;
		var _g4 = this.resource.geometry.indexBuffers.length;
		while(_g11 < _g4) {
			var i1 = _g11++;
			var mi = this.resource.geometry.materialIndices[i1];
			this.setMaterialConstants(g,shaderContext,materialContexts[mi]);
...
lue_resource_Resource.getSceneResource = function(name) {
	...
	if(cached == null) {
		var data = kha_Loader.the.getBlob(name).toString();
...
var lue_resource_MaterialContext = function(resource) {
	...
	if(resource.bind_textures.length > 0) {
		...
		while(_g1 < _g) {
			var i = _g1++;
			this.textures.push(kha_Loader.the.getImage(resource.bind_textures[i].name));
		}
	}
};
...
var lue_resource_ModelResource = function(resource) {
	...
	this.buildData(data,pa,true,na,true,uva,true,ca,true);
	...
	this.geometry = new lue_resource_Geometry(data,indices,materialIndices,pa,na,uva,usage);
	this.geometry.build(lue_resource_ShaderResource.getDefaultStructure(),lue_resource_ShaderResource.getDefaultStructureLength());
};
...
lue_resource_ModelResource.prototype = $extend(lue_resource_Resource.prototype,{
	...
	,buildData: function(data,pa,paEnabled,na,naEnabled,uva,uvaEnabled,ca,caEnabled,isSkinned) {
...
var lue_resource_Geometry = function(data,indices,materialIndices,positions,normals,uvs,usage) {
...
var lue_resource_ShaderResource = function(resource) {
	...
	lue_resource_Resource.call(this);
	if(resource == null) {
		haxe_Log.trace("Resource not found!",{ fileName : "ShaderResource.hx", lineNumber : 25, className : "lue.resource.ShaderResource", methodName : "new"});
		return;
	}
	this.resource = resource;
	var _g = 0;
	...
	while(_g < _g1.length) {
		var c = _g1[_g];
		++_g;
		this.contexts.push(new lue_resource_ShaderContext(c));
	}
};
...
lue_resource_ShaderResource.createDefaultStructure = function() {
	var structure = new kha_graphics4_VertexStructure();
	...
	structure.add("col",kha_graphics4_VertexData.Float4);
	return structure;
};
lue_resource_ShaderResource.getDefaultStructure = function() {
	if(lue_resource_ShaderResource.defaultStructure == null) lue_resource_ShaderResource.defaultStructure = lue_resource_ShaderResource.createDefaultStructure();
	return lue_resource_ShaderResource.defaultStructure;
};
lue_resource_ShaderResource.getDefaultStructureLength = function() {
	return 12;
};
...
lue_resource_ShaderContext.prototype = {
	...
	,addTexture: function(s) {
		this.textureUnits.push(this.program.getTextureUnit(s));
	}
...
var luegame_Game = function() {
	var materialRes = lue_resource_Resource.getMaterial("material_resource","");
	var lightRes = lue_resource_Resource.getLight("light_resource","");
	var camRes = lue_resource_Resource.getCamera("camera_resource","");
	this.cam = lue_Eg.addCameraNode(camRes);
	var light = lue_Eg.addLightNode(lightRes);
	lue_App.requestRender($bind(this,this.render));
};
$hxClasses["luegame.Game"] = luegame_Game;
luegame_Game.__name__ = ["luegame","Game"];
luegame_Game.prototype = {
	render: function(g) {
		lue_Eg.render(g,this.cam);
	}
	,__class__: luegame_Game
};
