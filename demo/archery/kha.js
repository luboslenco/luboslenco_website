...
var game_BowController = function() {
	this.rotatedY = 0.0;
	this.rotatedX = 0.0;
	this.state = 0;
	lue_trait_Trait.call(this);
	this.eul = new lue_math_Vec3(1.39626340159546358,0,0.506145483078355607);
	this.eul2 = new lue_math_Vec3();
	this.requestInit($bind(this,this.init));
	this.requestUpdate($bind(this,this.update));
};
$hxClasses["game.BowController"] = game_BowController;
game_BowController.__name__ = ["game","BowController"];
game_BowController.__super__ = lue_trait_Trait;
game_BowController.prototype = $extend(lue_trait_Trait.prototype,{
	state: null
	,camera: null
	,anim: null
	,arrow: null
	...
	,loopAudio: function() {
		var _g = this;
		lue_sys_Audio.playSound(kha_Assets.sounds.ambient,true);
		lue_sys_Tween.timer(16,function() {
			_g.loopAudio();
		});
	}
	,init: function() {
		this.loopAudio();
		this.camera = lue_node_Node.cameras[0];
		this.anim = this.node.getTrait(cycles_trait_Animation);
		this.arrow = js_Boot.__cast(lue_Eg.root.getChild("Arrow") , lue_node_ModelNode);
		this.target = js_Boot.__cast(lue_Eg.root.getChild("Target") , lue_node_ModelNode);
		this.handLeft = js_Boot.__cast(lue_Eg.root.getChild("Hand") , lue_node_ModelNode);
		this.handRight = js_Boot.__cast(lue_Eg.root.getChild("Hand2") , lue_node_ModelNode);
		this.bow = js_Boot.__cast(lue_Eg.root.getChild("limbs") , lue_node_ModelNode);
		this.arrowStartPos = this.arrow.transform.pos.clone();
		this.handRightStartPos = this.handRight.transform.pos.clone();
	}
	,update: function() {
		if(lue_sys_Input.started) {
			if(this.state == 0) {
				this.state = 1;
				this.anim.play("pull",false,1.0,$bind(this,this.onPullAnimated));
			} else if(this.state == 5) {
				this.arrow.transform.pos.set(this.arrowStartPos.x,this.arrowStartPos.y,this.arrowStartPos.z);
				this.arrow.transform.dirty = true;
				this.handRight.transform.pos.set(this.handRightStartPos.x,this.handRightStartPos.y,this.handRightStartPos.z);
				this.handRight.transform.dirty = true;
				this.state = 0;
			}
		} else if(lue_sys_Input.released) {
			if(this.state == 2) {
				this.state = 3;
				lue_sys_Audio.playSound(kha_Assets.sounds.swish);
				this.anim.play("release",false,6.0,$bind(this,this.onReleaseAnimated));
			}
		}
		if(lue_sys_Input.touch) {
			var up = this.camera.up();
			var right = this.camera.right();
			var look = this.camera.look();
			var dX = lue_sys_Input.deltaX / 200;
			var dY = lue_sys_Input.deltaY / 200;
			if(this.rotatedX + dX > 0.2) dX = 0.2 - this.rotatedX;
			if(this.rotatedX + dX < -0.2) dX = -0.2 - this.rotatedX;
			if(this.rotatedY + dY > 0.2) dY = 0.2 - this.rotatedY;
			if(this.rotatedY + dY < -0.2) dY = -0.2 - this.rotatedY;
			this.rotatedX += dX;
			this.rotatedY += dY;
			this.eul.x -= dY;
			...
			this.eul2.x -= dY;
			this.eul2.z -= dX;
			this.camera.transform.rot.setFromEuler(-this.eul.x,-this.eul.y,-this.eul.z,"XYZ");
			this.camera.transform.dirty = true;
			var q = new lue_math_Quat();
			q.setFromEuler(this.eul2.x,this.eul2.y,this.eul2.z,"XYZ");
			var m = new lue_math_Mat4();
			q.saveToMatrix(m);
			var m2 = new lue_math_Mat4();
			m2.initTranslate(-this.camera.transform.pos.x,-this.camera.transform.pos.y,-this.camera.transform.pos.z);
			m2.mult(m);
			m2.translate(this.camera.transform.pos.x,this.camera.transform.pos.y,this.camera.transform.pos.z);
			this.bow.transform.append = m2;
			this.handLeft.transform.append = m2;
			this.handRight.transform.append = m2;
			this.arrow.transform.append = m2;
			this.bow.transform.dirty = true;
			this.handLeft.transform.dirty = true;
			this.handRight.transform.dirty = true;
			this.arrow.transform.dirty = true;
		}
		if(this.state == 1) {
			this.moveArrow(-0.0036);
			this.moveRightHand(-0.0036);
		} else if(this.state == 3) {
			this.state = 4;
			this.arrowDist = 0;
			if(this.isPointInEllipse(0,0.01,0.095,0.095,this.rotatedX,this.rotatedY)) this.success = true; else this.success = false;
		}
		if(this.state == 4) {
			this.arrowDist += 0.2;
			this.moveArrow(0.2);
			if(this.arrowDist >= 1.8) this.state = 5;
		}
		if(this.state == 5 && !this.success) this.moveArrow(0.2);
	}
	,moveArrow: function(dist) {
		var ap = this.arrow.transform.pos;
		var dir = this.target.transform.pos.clone();
		dir.normalize();
		dir.multiplyScalar(dist * lue_sys_Time.delta * 100);
		ap.add(dir);
		this.arrow.transform.dirty = true;
	}
	,moveRightHand: function(dist) {
		var ap = this.handRight.transform.pos;
		var dir = this.target.transform.pos.clone();
		dir.normalize();
		dir.multiplyScalar(dist * lue_sys_Time.delta * 100);
		ap.add(dir);
	...
	,onPullAnimated: function() {
		this.state = 2;
		this.anim.pause();
	}
	,onReleaseAnimated: function() {
		this.anim.play("align",false,1.0);
	}
	,isPointInEllipse: function(cx,cy,rx,ry,lx,ly) {
		var nx = lx - cx;
		var ny = ly - cy;
		return nx * nx / (rx * rx) + ny * ny / (ry * ry) <= 1.0;
	}
	,__class__: game_BowController
});
...
var game_NodeTree = function() {
	cycles_trait_NodeExecutor.call(this);
	this.requestAdd($bind(this,this.add));
};
$hxClasses["game.NodeTree"] = game_NodeTree;
game_NodeTree.__name__ = ["game","NodeTree"];
game_NodeTree.__super__ = cycles_trait_NodeExecutor;
game_NodeTree.prototype = $extend(cycles_trait_NodeExecutor.prototype,{
	add: function() {
...
var kha_ImageList = function() {
	this.targetmapDescription = { files : ["Assets/targetmap.jpg"], original_height : 1024, type : "image", original_width : 1024, name : "targetmap"};
	this.targetmapName = "targetmap";
	this.targetmap = null;
	this.envmapDescription = { files : ["Assets/envmap.jpg"], original_height : 2048, type : "image", original_width : 4096, name : "envmap"};
	this.envmapName = "envmap";
	this.envmap = null;
	this.bowmapDescription = { files : ["Assets/bowmap.jpg"], original_height : 1024, type : "image", original_width : 1024, name : "bowmap"};
	this.bowmapName = "bowmap";
	this.bowmap = null;
};
...
kha_ImageList.prototype = {
	bowmap: null
	,bowmapName: null
	,bowmapDescription: null
	,bowmapLoad: function(done) {
		kha_Assets.loadImage("bowmap",function(image) {
			done();
		});
	}
	,bowmapUnload: function() {
		this.bowmap.unload();
		this.bowmap = null;
	}
	,envmap: null
	...
	,targetmap: null
	,targetmapName: null
	,targetmapDescription: null
	,targetmapLoad: function(done) {
		kha_Assets.loadImage("targetmap",function(image) {
			done();
		});
	}
	,targetmapUnload: function() {
		this.targetmap.unload();
		this.targetmap = null;
	}
	,__class__: kha_ImageList
};
var kha_SoundList = function() {
	this.swishDescription = { files : ["Assets/swish.ogg"], type : "sound", name : "swish"};
	this.swishName = "swish";
	this.swish = null;
	this.ambientDescription = { files : ["Assets/ambient.ogg"], type : "sound", name : "ambient"};
...
kha_SoundList.prototype = {
	...
	,ambientLoad: function(done) {
		kha_Assets.loadSound("ambient",function(sound) {
			done();
		});
	}
	,ambientUnload: function() {
		this.ambient.unload();
		this.ambient = null;
	}
	,swish: null
	,swishName: null
	,swishDescription: null
	,swishLoad: function(done) {
		kha_Assets.loadSound("swish",function(sound) {
			done();
		});
	}
	,swishUnload: function() {
		this.swish.unload();
		this.swish = null;
	}
...
var kha_BlobList = function() {
	this.pipeline_resourceDescription = { files : ["Assets/pipeline_resource.json"], type : "blob", name : "pipeline_resource"};
	this.pipeline_resourceName = "pipeline_resource";
	this.pipeline_resource = null;
	this.material_resourceDescription = { files : ["Assets/material_resource.json"], type : "blob", name : "material_resource"};
	this.material_resourceName = "material_resource";
	this.material_resource = null;
	this.geom_Plane_024Description = { files : ["Assets/geom_Plane_024.json"], type : "blob", name : "geom_Plane_024"};
	this.geom_Plane_024Name = "geom_Plane_024";
	this.geom_Plane_024 = null;
	this.geom_PlaneDescription = { files : ["Assets/geom_Plane.json"], type : "blob", name : "geom_Plane"};
	this.geom_PlaneName = "geom_Plane";
	this.geom_Plane = null;
	this.geom_CylinderDescription = { files : ["Assets/geom_Cylinder.json"], type : "blob", name : "geom_Cylinder"};
	this.geom_CylinderName = "geom_Cylinder";
	this.geom_Cylinder = null;
	this.geom_CubeDescription = { files : ["Assets/geom_Cube.json"], type : "blob", name : "geom_Cube"};
	this.geom_CubeName = "geom_Cube";
	this.geom_Cube = null;
	this.geom_Circle_002Description = { files : ["Assets/geom_Circle_002.json"], type : "blob", name : "geom_Circle_002"};
	this.geom_Circle_002Name = "geom_Circle_002";
	this.geom_Circle_002 = null;
	this.geom_Circle_001Description = { files : ["Assets/geom_Circle_001.json"], type : "blob", name : "geom_Circle_001"};
	this.geom_Circle_001Name = "geom_Circle_001";
	this.geom_Circle_001 = null;
	this.geom_CircleDescription = { files : ["Assets/geom_Circle.json"], type : "blob", name : "geom_Circle"};
	this.geom_CircleName = "geom_Circle";
	this.geom_Circle = null;
	this.env_map_resourceDescription = { files : ["Assets/env_map_resource.json"], type : "blob", name : "env_map_resource"};
	this.env_map_resourceName = "env_map_resource";
	this.env_map_resource = null;
	this.blender_resourceDescription = { files : ["Assets/blender_resource.json"], type : "blob", name : "blender_resource"};
	this.blender_resourceName = "blender_resource";
	this.blender_resource = null;
	this.SceneDescription = { files : ["Assets/Scene.json"], type : "blob", name : "Scene"};
...
kha_BlobList.prototype = {
	...
	,SceneLoad: function(done) {
		kha_Assets.loadBlob("Scene",function(blob) {
			done();
		});
	}
	,SceneUnload: function() {
		this.Scene.unload();
		this.Scene = null;
	}
	,blender_resource: null
	,blender_resourceName: null
	,blender_resourceDescription: null
	,blender_resourceLoad: function(done) {
		kha_Assets.loadBlob("blender_resource",function(blob) {
			done();
		});
	}
	,blender_resourceUnload: function() {
		this.blender_resource.unload();
		this.blender_resource = null;
	}
	,env_map_resource: null
	,env_map_resourceName: null
	,env_map_resourceDescription: null
	,env_map_resourceLoad: function(done) {
		kha_Assets.loadBlob("env_map_resource",function(blob) {
			done();
		});
	}
	,env_map_resourceUnload: function() {
		this.env_map_resource.unload();
		this.env_map_resource = null;
	}
	,geom_Circle: null
	,geom_CircleName: null
	,geom_CircleDescription: null
	,geom_CircleLoad: function(done) {
		kha_Assets.loadBlob("geom_Circle",function(blob) {
			done();
		});
	}
	,geom_CircleUnload: function() {
		this.geom_Circle.unload();
		this.geom_Circle = null;
	}
	,geom_Circle_001: null
	,geom_Circle_001Name: null
	,geom_Circle_001Description: null
	,geom_Circle_001Load: function(done) {
		kha_Assets.loadBlob("geom_Circle_001",function(blob) {
			done();
		});
	}
	,geom_Circle_001Unload: function() {
		this.geom_Circle_001.unload();
		this.geom_Circle_001 = null;
	}
	,geom_Circle_002: null
	,geom_Circle_002Name: null
	,geom_Circle_002Description: null
	,geom_Circle_002Load: function(done) {
		kha_Assets.loadBlob("geom_Circle_002",function(blob) {
			done();
		});
	}
	,geom_Circle_002Unload: function() {
		this.geom_Circle_002.unload();
		this.geom_Circle_002 = null;
	}
	,geom_Cube: null
	,geom_CubeName: null
	,geom_CubeDescription: null
	,geom_CubeLoad: function(done) {
		kha_Assets.loadBlob("geom_Cube",function(blob) {
			done();
		});
	}
	,geom_CubeUnload: function() {
		this.geom_Cube.unload();
		this.geom_Cube = null;
	}
	,geom_Cylinder: null
	,geom_CylinderName: null
	,geom_CylinderDescription: null
	,geom_CylinderLoad: function(done) {
		kha_Assets.loadBlob("geom_Cylinder",function(blob) {
			done();
		});
	}
	,geom_CylinderUnload: function() {
		this.geom_Cylinder.unload();
		this.geom_Cylinder = null;
	}
	,geom_Plane: null
	,geom_PlaneName: null
	,geom_PlaneDescription: null
	,geom_PlaneLoad: function(done) {
		kha_Assets.loadBlob("geom_Plane",function(blob) {
			done();
		});
	}
	,geom_PlaneUnload: function() {
		this.geom_Plane.unload();
		this.geom_Plane = null;
	}
	,geom_Plane_024: null
	,geom_Plane_024Name: null
	,geom_Plane_024Description: null
	,geom_Plane_024Load: function(done) {
		kha_Assets.loadBlob("geom_Plane_024",function(blob) {
			done();
		});
	}
	,geom_Plane_024Unload: function() {
		this.geom_Plane_024.unload();
		this.geom_Plane_024 = null;
	}
	,material_resource: null
	,material_resourceName: null
	,material_resourceDescription: null
	,material_resourceLoad: function(done) {
		kha_Assets.loadBlob("material_resource",function(blob) {
			done();
		});
	}
	,material_resourceUnload: function() {
		this.material_resource.unload();
		this.material_resource = null;
	}
	,pipeline_resource: null
	,pipeline_resourceName: null
	,pipeline_resourceDescription: null
	,pipeline_resourceLoad: function(done) {
		kha_Assets.loadBlob("pipeline_resource",function(blob) {
			done();
		});
	}
	,pipeline_resourceUnload: function() {
		this.pipeline_resource.unload();
		this.pipeline_resource = null;
	}
...
kha_Shaders.init = function() {
	...
	var data6 = Reflect.field(kha_Shaders,"blender_fragData");
	var bytes6 = haxe_Unserializer.run(data6);
	kha_Shaders.blender_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes6));
	var data7 = Reflect.field(kha_Shaders,"blender_vertData");
	var bytes7 = haxe_Unserializer.run(data7);
	kha_Shaders.blender_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes7));
	var data8 = Reflect.field(kha_Shaders,"blender_AlphaTest_fragData");
	var bytes8 = haxe_Unserializer.run(data8);
	kha_Shaders.blender_AlphaTest_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes8));
	var data9 = Reflect.field(kha_Shaders,"blender_AlphaTest_vertData");
	var bytes9 = haxe_Unserializer.run(data9);
	kha_Shaders.blender_AlphaTest_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes9));
	var data10 = Reflect.field(kha_Shaders,"blender_AlphaTest_Instancing_fragData");
	var bytes10 = haxe_Unserializer.run(data10);
	kha_Shaders.blender_AlphaTest_Instancing_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes10));
	var data11 = Reflect.field(kha_Shaders,"blender_AlphaTest_Instancing_vertData");
	var bytes11 = haxe_Unserializer.run(data11);
	kha_Shaders.blender_AlphaTest_Instancing_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes11));
	var data12 = Reflect.field(kha_Shaders,"blender_AlphaTest_Instancing_NormalMapping_fragData");
	var bytes12 = haxe_Unserializer.run(data12);
	kha_Shaders.blender_AlphaTest_Instancing_NormalMapping_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes12));
	var data13 = Reflect.field(kha_Shaders,"blender_AlphaTest_Instancing_NormalMapping_vertData");
	var bytes13 = haxe_Unserializer.run(data13);
	kha_Shaders.blender_AlphaTest_Instancing_NormalMapping_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes13));
	var data14 = Reflect.field(kha_Shaders,"blender_AlphaTest_Instancing_NormalMapping_Texturing_fragData");
	var bytes14 = haxe_Unserializer.run(data14);
	kha_Shaders.blender_AlphaTest_Instancing_NormalMapping_Texturing_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes14));
	var data15 = Reflect.field(kha_Shaders,"blender_AlphaTest_Instancing_NormalMapping_Texturing_vertData");
	var bytes15 = haxe_Unserializer.run(data15);
	kha_Shaders.blender_AlphaTest_Instancing_NormalMapping_Texturing_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes15));
	var data16 = Reflect.field(kha_Shaders,"blender_AlphaTest_Instancing_NormalMapping_Texturing_VCols_fragData");
	var bytes16 = haxe_Unserializer.run(data16);
	kha_Shaders.blender_AlphaTest_Instancing_NormalMapping_Texturing_VCols_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes16));
	var data17 = Reflect.field(kha_Shaders,"blender_AlphaTest_Instancing_NormalMapping_Texturing_VCols_vertData");
	var bytes17 = haxe_Unserializer.run(data17);
	kha_Shaders.blender_AlphaTest_Instancing_NormalMapping_Texturing_VCols_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes17));
	var data18 = Reflect.field(kha_Shaders,"blender_AlphaTest_Instancing_NormalMapping_VCols_fragData");
	var bytes18 = haxe_Unserializer.run(data18);
	kha_Shaders.blender_AlphaTest_Instancing_NormalMapping_VCols_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes18));
	var data19 = Reflect.field(kha_Shaders,"blender_AlphaTest_Instancing_NormalMapping_VCols_vertData");
	var bytes19 = haxe_Unserializer.run(data19);
	kha_Shaders.blender_AlphaTest_Instancing_NormalMapping_VCols_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes19));
	var data20 = Reflect.field(kha_Shaders,"blender_AlphaTest_Instancing_Texturing_fragData");
	var bytes20 = haxe_Unserializer.run(data20);
	kha_Shaders.blender_AlphaTest_Instancing_Texturing_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes20));
	var data21 = Reflect.field(kha_Shaders,"blender_AlphaTest_Instancing_Texturing_vertData");
	var bytes21 = haxe_Unserializer.run(data21);
	kha_Shaders.blender_AlphaTest_Instancing_Texturing_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes21));
	var data22 = Reflect.field(kha_Shaders,"blender_AlphaTest_Instancing_Texturing_VCols_fragData");
	var bytes22 = haxe_Unserializer.run(data22);
	kha_Shaders.blender_AlphaTest_Instancing_Texturing_VCols_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes22));
	var data23 = Reflect.field(kha_Shaders,"blender_AlphaTest_Instancing_Texturing_VCols_vertData");
	var bytes23 = haxe_Unserializer.run(data23);
	kha_Shaders.blender_AlphaTest_Instancing_Texturing_VCols_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes23));
	var data24 = Reflect.field(kha_Shaders,"blender_AlphaTest_Instancing_VCols_fragData");
	var bytes24 = haxe_Unserializer.run(data24);
	kha_Shaders.blender_AlphaTest_Instancing_VCols_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes24));
	var data25 = Reflect.field(kha_Shaders,"blender_AlphaTest_Instancing_VCols_vertData");
	var bytes25 = haxe_Unserializer.run(data25);
	kha_Shaders.blender_AlphaTest_Instancing_VCols_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes25));
	var data26 = Reflect.field(kha_Shaders,"blender_AlphaTest_NormalMapping_fragData");
	var bytes26 = haxe_Unserializer.run(data26);
	kha_Shaders.blender_AlphaTest_NormalMapping_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes26));
	var data27 = Reflect.field(kha_Shaders,"blender_AlphaTest_NormalMapping_vertData");
	var bytes27 = haxe_Unserializer.run(data27);
	kha_Shaders.blender_AlphaTest_NormalMapping_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes27));
	var data28 = Reflect.field(kha_Shaders,"blender_AlphaTest_NormalMapping_Texturing_fragData");
	var bytes28 = haxe_Unserializer.run(data28);
	kha_Shaders.blender_AlphaTest_NormalMapping_Texturing_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes28));
	var data29 = Reflect.field(kha_Shaders,"blender_AlphaTest_NormalMapping_Texturing_vertData");
	var bytes29 = haxe_Unserializer.run(data29);
	kha_Shaders.blender_AlphaTest_NormalMapping_Texturing_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes29));
	var data30 = Reflect.field(kha_Shaders,"blender_AlphaTest_NormalMapping_Texturing_VCols_fragData");
	var bytes30 = haxe_Unserializer.run(data30);
	kha_Shaders.blender_AlphaTest_NormalMapping_Texturing_VCols_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes30));
	var data31 = Reflect.field(kha_Shaders,"blender_AlphaTest_NormalMapping_Texturing_VCols_vertData");
	var bytes31 = haxe_Unserializer.run(data31);
	kha_Shaders.blender_AlphaTest_NormalMapping_Texturing_VCols_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes31));
	var data32 = Reflect.field(kha_Shaders,"blender_AlphaTest_NormalMapping_VCols_fragData");
	var bytes32 = haxe_Unserializer.run(data32);
	kha_Shaders.blender_AlphaTest_NormalMapping_VCols_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes32));
	var data33 = Reflect.field(kha_Shaders,"blender_AlphaTest_NormalMapping_VCols_vertData");
	var bytes33 = haxe_Unserializer.run(data33);
	kha_Shaders.blender_AlphaTest_NormalMapping_VCols_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes33));
	var data34 = Reflect.field(kha_Shaders,"blender_AlphaTest_Texturing_fragData");
	var bytes34 = haxe_Unserializer.run(data34);
	kha_Shaders.blender_AlphaTest_Texturing_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes34));
	var data35 = Reflect.field(kha_Shaders,"blender_AlphaTest_Texturing_vertData");
	var bytes35 = haxe_Unserializer.run(data35);
	kha_Shaders.blender_AlphaTest_Texturing_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes35));
	var data36 = Reflect.field(kha_Shaders,"blender_AlphaTest_Texturing_VCols_fragData");
	var bytes36 = haxe_Unserializer.run(data36);
	kha_Shaders.blender_AlphaTest_Texturing_VCols_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes36));
	var data37 = Reflect.field(kha_Shaders,"blender_AlphaTest_Texturing_VCols_vertData");
	var bytes37 = haxe_Unserializer.run(data37);
	kha_Shaders.blender_AlphaTest_Texturing_VCols_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes37));
	var data38 = Reflect.field(kha_Shaders,"blender_AlphaTest_VCols_fragData");
	var bytes38 = haxe_Unserializer.run(data38);
	kha_Shaders.blender_AlphaTest_VCols_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes38));
	var data39 = Reflect.field(kha_Shaders,"blender_AlphaTest_VCols_vertData");
	var bytes39 = haxe_Unserializer.run(data39);
	kha_Shaders.blender_AlphaTest_VCols_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes39));
	var data40 = Reflect.field(kha_Shaders,"blender_Instancing_fragData");
	var bytes40 = haxe_Unserializer.run(data40);
	kha_Shaders.blender_Instancing_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes40));
	var data41 = Reflect.field(kha_Shaders,"blender_Instancing_vertData");
	var bytes41 = haxe_Unserializer.run(data41);
	kha_Shaders.blender_Instancing_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes41));
	var data42 = Reflect.field(kha_Shaders,"blender_Instancing_NormalMapping_fragData");
	var bytes42 = haxe_Unserializer.run(data42);
	kha_Shaders.blender_Instancing_NormalMapping_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes42));
	var data43 = Reflect.field(kha_Shaders,"blender_Instancing_NormalMapping_vertData");
	var bytes43 = haxe_Unserializer.run(data43);
	kha_Shaders.blender_Instancing_NormalMapping_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes43));
	var data44 = Reflect.field(kha_Shaders,"blender_Instancing_NormalMapping_Texturing_fragData");
	var bytes44 = haxe_Unserializer.run(data44);
	kha_Shaders.blender_Instancing_NormalMapping_Texturing_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes44));
	var data45 = Reflect.field(kha_Shaders,"blender_Instancing_NormalMapping_Texturing_vertData");
	var bytes45 = haxe_Unserializer.run(data45);
	kha_Shaders.blender_Instancing_NormalMapping_Texturing_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes45));
	var data46 = Reflect.field(kha_Shaders,"blender_Instancing_NormalMapping_Texturing_VCols_fragData");
	var bytes46 = haxe_Unserializer.run(data46);
	kha_Shaders.blender_Instancing_NormalMapping_Texturing_VCols_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes46));
	var data47 = Reflect.field(kha_Shaders,"blender_Instancing_NormalMapping_Texturing_VCols_vertData");
	var bytes47 = haxe_Unserializer.run(data47);
	kha_Shaders.blender_Instancing_NormalMapping_Texturing_VCols_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes47));
	var data48 = Reflect.field(kha_Shaders,"blender_Instancing_NormalMapping_VCols_fragData");
	var bytes48 = haxe_Unserializer.run(data48);
	kha_Shaders.blender_Instancing_NormalMapping_VCols_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes48));
	var data49 = Reflect.field(kha_Shaders,"blender_Instancing_NormalMapping_VCols_vertData");
	var bytes49 = haxe_Unserializer.run(data49);
	kha_Shaders.blender_Instancing_NormalMapping_VCols_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes49));
	var data50 = Reflect.field(kha_Shaders,"blender_Instancing_Texturing_fragData");
	var bytes50 = haxe_Unserializer.run(data50);
	kha_Shaders.blender_Instancing_Texturing_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes50));
	var data51 = Reflect.field(kha_Shaders,"blender_Instancing_Texturing_vertData");
	var bytes51 = haxe_Unserializer.run(data51);
	kha_Shaders.blender_Instancing_Texturing_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes51));
	var data52 = Reflect.field(kha_Shaders,"blender_Instancing_Texturing_VCols_fragData");
	var bytes52 = haxe_Unserializer.run(data52);
	kha_Shaders.blender_Instancing_Texturing_VCols_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes52));
	var data53 = Reflect.field(kha_Shaders,"blender_Instancing_Texturing_VCols_vertData");
	var bytes53 = haxe_Unserializer.run(data53);
	kha_Shaders.blender_Instancing_Texturing_VCols_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes53));
	var data54 = Reflect.field(kha_Shaders,"blender_Instancing_VCols_fragData");
	var bytes54 = haxe_Unserializer.run(data54);
	kha_Shaders.blender_Instancing_VCols_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes54));
	var data55 = Reflect.field(kha_Shaders,"blender_Instancing_VCols_vertData");
	var bytes55 = haxe_Unserializer.run(data55);
	kha_Shaders.blender_Instancing_VCols_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes55));
	var data56 = Reflect.field(kha_Shaders,"blender_NormalMapping_fragData");
	var bytes56 = haxe_Unserializer.run(data56);
	kha_Shaders.blender_NormalMapping_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes56));
	var data57 = Reflect.field(kha_Shaders,"blender_NormalMapping_vertData");
	var bytes57 = haxe_Unserializer.run(data57);
	kha_Shaders.blender_NormalMapping_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes57));
	var data58 = Reflect.field(kha_Shaders,"blender_NormalMapping_Texturing_fragData");
	var bytes58 = haxe_Unserializer.run(data58);
	kha_Shaders.blender_NormalMapping_Texturing_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes58));
	var data59 = Reflect.field(kha_Shaders,"blender_NormalMapping_Texturing_vertData");
	var bytes59 = haxe_Unserializer.run(data59);
	kha_Shaders.blender_NormalMapping_Texturing_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes59));
	var data60 = Reflect.field(kha_Shaders,"blender_NormalMapping_Texturing_VCols_fragData");
	var bytes60 = haxe_Unserializer.run(data60);
	kha_Shaders.blender_NormalMapping_Texturing_VCols_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes60));
	var data61 = Reflect.field(kha_Shaders,"blender_NormalMapping_Texturing_VCols_vertData");
	var bytes61 = haxe_Unserializer.run(data61);
	kha_Shaders.blender_NormalMapping_Texturing_VCols_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes61));
	var data62 = Reflect.field(kha_Shaders,"blender_NormalMapping_VCols_fragData");
	var bytes62 = haxe_Unserializer.run(data62);
	kha_Shaders.blender_NormalMapping_VCols_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes62));
	var data63 = Reflect.field(kha_Shaders,"blender_NormalMapping_VCols_vertData");
	var bytes63 = haxe_Unserializer.run(data63);
	kha_Shaders.blender_NormalMapping_VCols_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes63));
	var data64 = Reflect.field(kha_Shaders,"blender_Texturing_fragData");
	var bytes64 = haxe_Unserializer.run(data64);
	kha_Shaders.blender_Texturing_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes64));
	var data65 = Reflect.field(kha_Shaders,"blender_Texturing_vertData");
	var bytes65 = haxe_Unserializer.run(data65);
	kha_Shaders.blender_Texturing_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes65));
	var data66 = Reflect.field(kha_Shaders,"blender_Texturing_VCols_fragData");
	var bytes66 = haxe_Unserializer.run(data66);
	kha_Shaders.blender_Texturing_VCols_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes66));
	var data67 = Reflect.field(kha_Shaders,"blender_Texturing_VCols_vertData");
	var bytes67 = haxe_Unserializer.run(data67);
	kha_Shaders.blender_Texturing_VCols_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes67));
	var data68 = Reflect.field(kha_Shaders,"blender_VCols_fragData");
	var bytes68 = haxe_Unserializer.run(data68);
	kha_Shaders.blender_VCols_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes68));
	var data69 = Reflect.field(kha_Shaders,"blender_VCols_vertData");
	var bytes69 = haxe_Unserializer.run(data69);
	kha_Shaders.blender_VCols_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes69));
	var data70 = Reflect.field(kha_Shaders,"env_map_fragData");
	var bytes70 = haxe_Unserializer.run(data70);
	kha_Shaders.env_map_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes70));
	var data71 = Reflect.field(kha_Shaders,"env_map_vertData");
	var bytes71 = haxe_Unserializer.run(data71);
	kha_Shaders.env_map_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes71));
	var data72 = Reflect.field(kha_Shaders,"shadow_map_fragData");
	var bytes72 = haxe_Unserializer.run(data72);
	kha_Shaders.shadow_map_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes72));
	var data73 = Reflect.field(kha_Shaders,"shadow_map_vertData");
	var bytes73 = haxe_Unserializer.run(data73);
	kha_Shaders.shadow_map_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes73));
	var data74 = Reflect.field(kha_Shaders,"shadow_map_AlphaTest_fragData");
	var bytes74 = haxe_Unserializer.run(data74);
	kha_Shaders.shadow_map_AlphaTest_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes74));
	var data75 = Reflect.field(kha_Shaders,"shadow_map_AlphaTest_vertData");
	var bytes75 = haxe_Unserializer.run(data75);
	kha_Shaders.shadow_map_AlphaTest_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes75));
	var data76 = Reflect.field(kha_Shaders,"shadow_map_AlphaTest_Instancing_fragData");
	var bytes76 = haxe_Unserializer.run(data76);
	kha_Shaders.shadow_map_AlphaTest_Instancing_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes76));
	var data77 = Reflect.field(kha_Shaders,"shadow_map_AlphaTest_Instancing_vertData");
	var bytes77 = haxe_Unserializer.run(data77);
	kha_Shaders.shadow_map_AlphaTest_Instancing_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes77));
	var data78 = Reflect.field(kha_Shaders,"shadow_map_AlphaTest_Instancing_NormalMapping_fragData");
	var bytes78 = haxe_Unserializer.run(data78);
	kha_Shaders.shadow_map_AlphaTest_Instancing_NormalMapping_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes78));
	var data79 = Reflect.field(kha_Shaders,"shadow_map_AlphaTest_Instancing_NormalMapping_vertData");
	var bytes79 = haxe_Unserializer.run(data79);
	kha_Shaders.shadow_map_AlphaTest_Instancing_NormalMapping_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes79));
	var data80 = Reflect.field(kha_Shaders,"shadow_map_AlphaTest_Instancing_NormalMapping_Texturing_fragData");
	var bytes80 = haxe_Unserializer.run(data80);
	kha_Shaders.shadow_map_AlphaTest_Instancing_NormalMapping_Texturing_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes80));
	var data81 = Reflect.field(kha_Shaders,"shadow_map_AlphaTest_Instancing_NormalMapping_Texturing_vertData");
	var bytes81 = haxe_Unserializer.run(data81);
	kha_Shaders.shadow_map_AlphaTest_Instancing_NormalMapping_Texturing_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes81));
	var data82 = Reflect.field(kha_Shaders,"shadow_map_AlphaTest_Instancing_NormalMapping_Texturing_VCols_fragData");
	var bytes82 = haxe_Unserializer.run(data82);
	kha_Shaders.shadow_map_AlphaTest_Instancing_NormalMapping_Texturing_VCols_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes82));
	var data83 = Reflect.field(kha_Shaders,"shadow_map_AlphaTest_Instancing_NormalMapping_Texturing_VCols_vertData");
	var bytes83 = haxe_Unserializer.run(data83);
	kha_Shaders.shadow_map_AlphaTest_Instancing_NormalMapping_Texturing_VCols_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes83));
	var data84 = Reflect.field(kha_Shaders,"shadow_map_AlphaTest_Instancing_NormalMapping_VCols_fragData");
	var bytes84 = haxe_Unserializer.run(data84);
	kha_Shaders.shadow_map_AlphaTest_Instancing_NormalMapping_VCols_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes84));
	var data85 = Reflect.field(kha_Shaders,"shadow_map_AlphaTest_Instancing_NormalMapping_VCols_vertData");
	var bytes85 = haxe_Unserializer.run(data85);
	kha_Shaders.shadow_map_AlphaTest_Instancing_NormalMapping_VCols_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes85));
	var data86 = Reflect.field(kha_Shaders,"shadow_map_AlphaTest_Instancing_Texturing_fragData");
	var bytes86 = haxe_Unserializer.run(data86);
	kha_Shaders.shadow_map_AlphaTest_Instancing_Texturing_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes86));
	var data87 = Reflect.field(kha_Shaders,"shadow_map_AlphaTest_Instancing_Texturing_vertData");
	var bytes87 = haxe_Unserializer.run(data87);
	kha_Shaders.shadow_map_AlphaTest_Instancing_Texturing_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes87));
	var data88 = Reflect.field(kha_Shaders,"shadow_map_AlphaTest_Instancing_Texturing_VCols_fragData");
	var bytes88 = haxe_Unserializer.run(data88);
	kha_Shaders.shadow_map_AlphaTest_Instancing_Texturing_VCols_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes88));
	var data89 = Reflect.field(kha_Shaders,"shadow_map_AlphaTest_Instancing_Texturing_VCols_vertData");
	var bytes89 = haxe_Unserializer.run(data89);
	kha_Shaders.shadow_map_AlphaTest_Instancing_Texturing_VCols_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes89));
	var data90 = Reflect.field(kha_Shaders,"shadow_map_AlphaTest_Instancing_VCols_fragData");
	var bytes90 = haxe_Unserializer.run(data90);
	kha_Shaders.shadow_map_AlphaTest_Instancing_VCols_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes90));
	var data91 = Reflect.field(kha_Shaders,"shadow_map_AlphaTest_Instancing_VCols_vertData");
	var bytes91 = haxe_Unserializer.run(data91);
	kha_Shaders.shadow_map_AlphaTest_Instancing_VCols_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes91));
	var data92 = Reflect.field(kha_Shaders,"shadow_map_AlphaTest_NormalMapping_fragData");
	var bytes92 = haxe_Unserializer.run(data92);
	kha_Shaders.shadow_map_AlphaTest_NormalMapping_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes92));
	var data93 = Reflect.field(kha_Shaders,"shadow_map_AlphaTest_NormalMapping_vertData");
	var bytes93 = haxe_Unserializer.run(data93);
	kha_Shaders.shadow_map_AlphaTest_NormalMapping_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes93));
	var data94 = Reflect.field(kha_Shaders,"shadow_map_AlphaTest_NormalMapping_Texturing_fragData");
	var bytes94 = haxe_Unserializer.run(data94);
	kha_Shaders.shadow_map_AlphaTest_NormalMapping_Texturing_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes94));
	var data95 = Reflect.field(kha_Shaders,"shadow_map_AlphaTest_NormalMapping_Texturing_vertData");
	var bytes95 = haxe_Unserializer.run(data95);
	kha_Shaders.shadow_map_AlphaTest_NormalMapping_Texturing_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes95));
	var data96 = Reflect.field(kha_Shaders,"shadow_map_AlphaTest_NormalMapping_Texturing_VCols_fragData");
	var bytes96 = haxe_Unserializer.run(data96);
	kha_Shaders.shadow_map_AlphaTest_NormalMapping_Texturing_VCols_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes96));
	var data97 = Reflect.field(kha_Shaders,"shadow_map_AlphaTest_NormalMapping_Texturing_VCols_vertData");
	var bytes97 = haxe_Unserializer.run(data97);
	kha_Shaders.shadow_map_AlphaTest_NormalMapping_Texturing_VCols_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes97));
	var data98 = Reflect.field(kha_Shaders,"shadow_map_AlphaTest_NormalMapping_VCols_fragData");
	var bytes98 = haxe_Unserializer.run(data98);
	kha_Shaders.shadow_map_AlphaTest_NormalMapping_VCols_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes98));
	var data99 = Reflect.field(kha_Shaders,"shadow_map_AlphaTest_NormalMapping_VCols_vertData");
	var bytes99 = haxe_Unserializer.run(data99);
	kha_Shaders.shadow_map_AlphaTest_NormalMapping_VCols_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes99));
	var data100 = Reflect.field(kha_Shaders,"shadow_map_AlphaTest_Texturing_fragData");
	var bytes100 = haxe_Unserializer.run(data100);
	kha_Shaders.shadow_map_AlphaTest_Texturing_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes100));
	var data101 = Reflect.field(kha_Shaders,"shadow_map_AlphaTest_Texturing_vertData");
	var bytes101 = haxe_Unserializer.run(data101);
	kha_Shaders.shadow_map_AlphaTest_Texturing_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes101));
	var data102 = Reflect.field(kha_Shaders,"shadow_map_AlphaTest_Texturing_VCols_fragData");
	var bytes102 = haxe_Unserializer.run(data102);
	kha_Shaders.shadow_map_AlphaTest_Texturing_VCols_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes102));
	var data103 = Reflect.field(kha_Shaders,"shadow_map_AlphaTest_Texturing_VCols_vertData");
	var bytes103 = haxe_Unserializer.run(data103);
	kha_Shaders.shadow_map_AlphaTest_Texturing_VCols_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes103));
	var data104 = Reflect.field(kha_Shaders,"shadow_map_AlphaTest_VCols_fragData");
	var bytes104 = haxe_Unserializer.run(data104);
	kha_Shaders.shadow_map_AlphaTest_VCols_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes104));
	var data105 = Reflect.field(kha_Shaders,"shadow_map_AlphaTest_VCols_vertData");
	var bytes105 = haxe_Unserializer.run(data105);
	kha_Shaders.shadow_map_AlphaTest_VCols_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes105));
	var data106 = Reflect.field(kha_Shaders,"shadow_map_Instancing_fragData");
	var bytes106 = haxe_Unserializer.run(data106);
	kha_Shaders.shadow_map_Instancing_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes106));
	var data107 = Reflect.field(kha_Shaders,"shadow_map_Instancing_vertData");
	var bytes107 = haxe_Unserializer.run(data107);
	kha_Shaders.shadow_map_Instancing_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes107));
	var data108 = Reflect.field(kha_Shaders,"shadow_map_Instancing_NormalMapping_fragData");
	var bytes108 = haxe_Unserializer.run(data108);
	kha_Shaders.shadow_map_Instancing_NormalMapping_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes108));
	var data109 = Reflect.field(kha_Shaders,"shadow_map_Instancing_NormalMapping_vertData");
	var bytes109 = haxe_Unserializer.run(data109);
	kha_Shaders.shadow_map_Instancing_NormalMapping_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes109));
	var data110 = Reflect.field(kha_Shaders,"shadow_map_Instancing_NormalMapping_Texturing_fragData");
	var bytes110 = haxe_Unserializer.run(data110);
	kha_Shaders.shadow_map_Instancing_NormalMapping_Texturing_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes110));
	var data111 = Reflect.field(kha_Shaders,"shadow_map_Instancing_NormalMapping_Texturing_vertData");
	var bytes111 = haxe_Unserializer.run(data111);
	kha_Shaders.shadow_map_Instancing_NormalMapping_Texturing_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes111));
	var data112 = Reflect.field(kha_Shaders,"shadow_map_Instancing_NormalMapping_Texturing_VCols_fragData");
	var bytes112 = haxe_Unserializer.run(data112);
	kha_Shaders.shadow_map_Instancing_NormalMapping_Texturing_VCols_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes112));
	var data113 = Reflect.field(kha_Shaders,"shadow_map_Instancing_NormalMapping_Texturing_VCols_vertData");
	var bytes113 = haxe_Unserializer.run(data113);
	kha_Shaders.shadow_map_Instancing_NormalMapping_Texturing_VCols_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes113));
	var data114 = Reflect.field(kha_Shaders,"shadow_map_Instancing_NormalMapping_VCols_fragData");
	var bytes114 = haxe_Unserializer.run(data114);
	kha_Shaders.shadow_map_Instancing_NormalMapping_VCols_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes114));
	var data115 = Reflect.field(kha_Shaders,"shadow_map_Instancing_NormalMapping_VCols_vertData");
	var bytes115 = haxe_Unserializer.run(data115);
	kha_Shaders.shadow_map_Instancing_NormalMapping_VCols_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes115));
	var data116 = Reflect.field(kha_Shaders,"shadow_map_Instancing_Texturing_fragData");
	var bytes116 = haxe_Unserializer.run(data116);
	kha_Shaders.shadow_map_Instancing_Texturing_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes116));
	var data117 = Reflect.field(kha_Shaders,"shadow_map_Instancing_Texturing_vertData");
	var bytes117 = haxe_Unserializer.run(data117);
	kha_Shaders.shadow_map_Instancing_Texturing_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes117));
	var data118 = Reflect.field(kha_Shaders,"shadow_map_Instancing_Texturing_VCols_fragData");
	var bytes118 = haxe_Unserializer.run(data118);
	kha_Shaders.shadow_map_Instancing_Texturing_VCols_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes118));
	var data119 = Reflect.field(kha_Shaders,"shadow_map_Instancing_Texturing_VCols_vertData");
	var bytes119 = haxe_Unserializer.run(data119);
	kha_Shaders.shadow_map_Instancing_Texturing_VCols_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes119));
	var data120 = Reflect.field(kha_Shaders,"shadow_map_Instancing_VCols_fragData");
	var bytes120 = haxe_Unserializer.run(data120);
	kha_Shaders.shadow_map_Instancing_VCols_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes120));
	var data121 = Reflect.field(kha_Shaders,"shadow_map_Instancing_VCols_vertData");
	var bytes121 = haxe_Unserializer.run(data121);
	kha_Shaders.shadow_map_Instancing_VCols_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes121));
	var data122 = Reflect.field(kha_Shaders,"shadow_map_NormalMapping_fragData");
	var bytes122 = haxe_Unserializer.run(data122);
	kha_Shaders.shadow_map_NormalMapping_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes122));
	var data123 = Reflect.field(kha_Shaders,"shadow_map_NormalMapping_vertData");
	var bytes123 = haxe_Unserializer.run(data123);
	kha_Shaders.shadow_map_NormalMapping_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes123));
	var data124 = Reflect.field(kha_Shaders,"shadow_map_NormalMapping_Texturing_fragData");
	var bytes124 = haxe_Unserializer.run(data124);
	kha_Shaders.shadow_map_NormalMapping_Texturing_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes124));
	var data125 = Reflect.field(kha_Shaders,"shadow_map_NormalMapping_Texturing_vertData");
	var bytes125 = haxe_Unserializer.run(data125);
	kha_Shaders.shadow_map_NormalMapping_Texturing_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes125));
	var data126 = Reflect.field(kha_Shaders,"shadow_map_NormalMapping_Texturing_VCols_fragData");
	var bytes126 = haxe_Unserializer.run(data126);
	kha_Shaders.shadow_map_NormalMapping_Texturing_VCols_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes126));
	var data127 = Reflect.field(kha_Shaders,"shadow_map_NormalMapping_Texturing_VCols_vertData");
	var bytes127 = haxe_Unserializer.run(data127);
	kha_Shaders.shadow_map_NormalMapping_Texturing_VCols_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes127));
	var data128 = Reflect.field(kha_Shaders,"shadow_map_NormalMapping_VCols_fragData");
	var bytes128 = haxe_Unserializer.run(data128);
	kha_Shaders.shadow_map_NormalMapping_VCols_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes128));
	var data129 = Reflect.field(kha_Shaders,"shadow_map_NormalMapping_VCols_vertData");
	var bytes129 = haxe_Unserializer.run(data129);
	kha_Shaders.shadow_map_NormalMapping_VCols_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes129));
	var data130 = Reflect.field(kha_Shaders,"shadow_map_Texturing_fragData");
	var bytes130 = haxe_Unserializer.run(data130);
	kha_Shaders.shadow_map_Texturing_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes130));
	var data131 = Reflect.field(kha_Shaders,"shadow_map_Texturing_vertData");
	var bytes131 = haxe_Unserializer.run(data131);
	kha_Shaders.shadow_map_Texturing_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes131));
	var data132 = Reflect.field(kha_Shaders,"shadow_map_Texturing_VCols_fragData");
	var bytes132 = haxe_Unserializer.run(data132);
	kha_Shaders.shadow_map_Texturing_VCols_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes132));
	var data133 = Reflect.field(kha_Shaders,"shadow_map_Texturing_VCols_vertData");
	var bytes133 = haxe_Unserializer.run(data133);
	kha_Shaders.shadow_map_Texturing_VCols_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes133));
	var data134 = Reflect.field(kha_Shaders,"shadow_map_VCols_fragData");
	var bytes134 = haxe_Unserializer.run(data134);
	kha_Shaders.shadow_map_VCols_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes134));
	var data135 = Reflect.field(kha_Shaders,"shadow_map_VCols_vertData");
	var bytes135 = haxe_Unserializer.run(data135);
	kha_Shaders.shadow_map_VCols_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes135));
	var data136 = Reflect.field(kha_Shaders,"final_fragData");
	var bytes136 = haxe_Unserializer.run(data136);
	kha_Shaders.final_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes136));
	var data137 = Reflect.field(kha_Shaders,"final_vertData");
	var bytes137 = haxe_Unserializer.run(data137);
	kha_Shaders.final_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes137));
};
...
kha_SystemImpl.checkGamepadButton = function(pad,num,button) {
	...
	animate1 = function(timestamp) {
		...
		if(sysGamepads != null) {
			...
			while(_g1 < _g) {
				...
				if(pad != null) {
					kha_SystemImpl.checkGamepadButton(pad,0,kha_Button.BUTTON_1);
					kha_SystemImpl.checkGamepadButton(pad,1,kha_Button.BUTTON_2);
					kha_SystemImpl.checkGamepadButton(pad,12,kha_Button.UP);
					kha_SystemImpl.checkGamepadButton(pad,13,kha_Button.DOWN);
					kha_SystemImpl.checkGamepadButton(pad,14,kha_Button.LEFT);
					kha_SystemImpl.checkGamepadButton(pad,15,kha_Button.RIGHT);
...
kha_graphics2_truetype_StbTruetype.stbtt_FindGlyphIndex = function(info,unicode_codepoint) {
	...
	if(format == 0) {
		...
		if(js_Boot.__cast(unicode_codepoint , Int) >= first && js_Boot.__cast(unicode_codepoint , Int) < first + count) return kha_graphics2_truetype_StbTruetype.ttUSHORT(data,index_map + 10 + (unicode_codepoint - first) * 2);
		return 0;
	} else if(format == 2) {
		...
		if(offset == 0) return js_Boot.__cast(unicode_codepoint + kha_graphics2_truetype_StbTruetype.ttSHORT(data,index_map + 14 + segcount * 4 + 2 + 2 * item) , Int);
		...
		while(low < high) {
			...
			if(js_Boot.__cast(unicode_codepoint , Int) < start_char) high = mid; else if(js_Boot.__cast(unicode_codepoint , Int) > end_char) low = mid + 1; else {
...
kha_graphics2_truetype_StbTruetype.stbtt_GetGlyphShape = function(info,glyph_index) {
	...
	if(numberOfContours > 0) {
		...
		while(_g3 < n) {
			...
			vertices[off + i3].x = js_Boot.__cast(x , Int);
		}
		y = 0;
		var _g4 = 0;
		while(_g4 < n) {
			...
			vertices[off + i4].y = js_Boot.__cast(y , Int);
		}
		num_vertices = 0;
		...
		while(i < n) {
			...
			x = js_Boot.__cast(vertices[off + i].x , Int);
			y = js_Boot.__cast(vertices[off + i].y , Int);
			if(next_move == i) {
				...
				if(start_off) {
					...
					if((vertices[off + i + 1].type & 1) == 0) {
						sx = x + js_Boot.__cast(vertices[off + i + 1].x , Int) >> 1;
						sy = y + js_Boot.__cast(vertices[off + i + 1].y , Int) >> 1;
					} else {
						sx = js_Boot.__cast(vertices[off + i + 1].x , Int);
						sy = js_Boot.__cast(vertices[off + i + 1].y , Int);
						++i;
					}
				} else {
					sx = x;
		...
		while(more != 0) {
			...
			if(comp_num_verts > 0) {
				...
				while(_g5 < comp_num_verts) {
					...
					x1 = v.x;
					y1 = v.y;
					v.x = js_Boot.__cast(m1 * (mtx0 * x1 + mtx2 * y1 + mtx4) , Int);
					v.y = js_Boot.__cast(n3 * (mtx1 * x1 + mtx3 * y1 + mtx5) , Int);
					x1 = v.cx;
					y1 = v.cy;
					v.cx = js_Boot.__cast(m1 * (mtx0 * x1 + mtx2 * y1 + mtx4) , Int);
					v.cy = js_Boot.__cast(n3 * (mtx1 * x1 + mtx3 * y1 + mtx5) , Int);
				}
				var this2;
...
kha_graphics2_truetype_StbTruetype.stbtt_ScaleForPixelHeight = function(info,height) {
	...
	return js_Boot.__cast(height / fheight , Float);
};
		...
		if(e.fdx == 0) {
			...
			if(x_top >= 0 && x_bottom >= 0 && x_top < len && x_bottom < len) {
				...
				while(_g6 < len) {
					...
					var y0 = y_top;
					var x12;
					x12 = js_Boot.__cast(x4 , Float);
					var x21;
					x21 = js_Boot.__cast(x4 + 1 , Float);
					var x31 = xb;
					var y3 = y_bottom;
					var y1;
					var y2;
...
kha_graphics2_truetype_StbTruetype.stbtt__rasterize_sorted_edges = function(result,e,n,vsubsample,off_x,off_y) {
	...
	y = off_y;
	e[eIndex + n].y0 = js_Boot.__cast(off_y + result.h + 1 , Float);
	while(j < result.h) {
		...
		while(_g12 < _g3) {
			...
			m = k | 0;
			if(m > 255) m = 255;
			result.pixels.writeU8(result.pixels_offset + j * result.stride + i3,js_Boot.__cast(m , Int));
		}
		step_parent = null;
		step_value = active;
		while(step_value != null) {
...
kha_graphics2_truetype_StbTruetype.stbtt_BakeFontBitmap = function(data,offset,pixel_height,pixels,pw,ph,first_char,num_chars,chardata) {
	...
	while(_g2 < num_chars) {
		...
		chardata[i1].x0 = js_Boot.__cast(x , Int);
		chardata[i1].y0 = js_Boot.__cast(y , Int);
		chardata[i1].x1 = js_Boot.__cast(x + gw , Int);
		chardata[i1].y1 = js_Boot.__cast(y + gh , Int);
		chardata[i1].xadvance = scale * advance;
		chardata[i1].xoff = js_Boot.__cast(x0 , Float);
		chardata[i1].yoff = js_Boot.__cast(y0 , Float);
		x = x + gw + 1;
...
kha_js_graphics4_Graphics.prototype = {
	...
	,setPipeline: function(pipe) {
		...
		this.setDepthMode(true,pipe.depthMode);
...
lue_node_Node.traverseNodes = function(resource,name,parent,nodes) {
	...
	while(_g < nodes.length) {
		...
		if(n.type == "camera_node") node = lue_Eg.addCameraNode(lue_resource_Resource.getCamera(name,n.object_ref),parent); else if(n.type == "light_node") node = lue_Eg.addLightNode(lue_resource_Resource.getLight(name,n.object_ref),parent); else if(n.type == "geometry_node") {
			...
			node = lue_Eg.addModelNode(lue_resource_Resource.getModel(object_file,object_ref,resource.nodes),materials,parent);
		} else if(n.type == "node") node = lue_Eg.addNode(parent);
		if(node != null) {
			node.id = n.id;
...
lue_node_CameraNode.prototype = $extend(lue_node_Node.prototype,{
	...
	,sphereInFrustum: function(t,radius) {
		...
		while(_g < 6) {
			var i = _g++;
			var vpos = new lue_math_Vec3(t.pos.x,t.pos.y,t.pos.z);
			var sphere = new lue_math_Sphere(vpos,radius);
...
lue_resource_Resource.getSceneResource = function(name) {
	...
	if(cached == null) {
		var data = Reflect.field(kha_Assets.blobs,name).toString();
...
var lue_resource_ModelResource = function(resource) {
	...
	if(resource.mesh.instance_offsets != null) {
		...
		this.geometry.instanceCount = resource.mesh.instance_offsets.length / 3 | 0;
		var structure = new kha_graphics4_VertexStructure();
		...
		while(_g11 < _g2) {
			var i = _g11++;
			vertices[i] = resource.mesh.instance_offsets[i];
		}
		vb.unlock();
...
lue_resource_Geometry.prototype = {
	...
	,calculateAABB: function() {
		this.aabbMin = new lue_math_Vec3(-0.1,-0.1,-0.1);
		this.aabbMax = new lue_math_Vec3(0.1,0.1,0.1);
		this.size = new lue_math_Vec3();
		var i = 0;
...
game_BowController.StateIdle = 0;
game_BowController.StatePull = 1;
game_BowController.StateAim = 2;
game_BowController.StateRelease = 3;
game_BowController.StateFlying = 4;
game_BowController.StateDone = 5;
...
kha_Shaders.blender_fragData = "s3804:%";
kha_Shaders.blender_vertData = "s1166:";
...
kha_Shaders.blender_AlphaTest_vertData = "s1166:";
...
kha_Shaders.blender_AlphaTest_Instancing_vertData = "s1222:";
...
kha_Shaders.blender_AlphaTest_Instancing_NormalMapping_vertData = "s1695:";
...
kha_Shaders.blender_AlphaTest_Instancing_NormalMapping_Texturing_vertData = "s1695:";
...
kha_Shaders.blender_AlphaTest_Instancing_NormalMapping_Texturing_VCols_vertData = "s1755:";
...
kha_Shaders.blender_AlphaTest_Instancing_NormalMapping_VCols_vertData = "s1755:";
...
kha_Shaders.blender_AlphaTest_Instancing_Texturing_vertData = "s1319:";
...
kha_Shaders.blender_AlphaTest_Instancing_Texturing_VCols_vertData = "s1379:";
...
kha_Shaders.blender_AlphaTest_Instancing_VCols_vertData = "s1282:";
...
kha_Shaders.blender_AlphaTest_NormalMapping_vertData = "s1639:";
...
kha_Shaders.blender_AlphaTest_NormalMapping_Texturing_vertData = "s1639:";
kha_Shaders.blender_AlphaTest_NormalMapping_Texturing_VCols_fragData = "s4132:%";
kha_Shaders.blender_AlphaTest_NormalMapping_Texturing_VCols_vertData = "s1699:";
...
kha_Shaders.blender_AlphaTest_Texturing_fragData = "s3975:";
kha_Shaders.blender_AlphaTest_Texturing_vertData = "s1263:";
kha_Shaders.blender_AlphaTest_Texturing_VCols_fragData = "s3984:";
kha_Shaders.blender_AlphaTest_Texturing_VCols_vertData = "s1323:";
...
kha_Shaders.blender_AlphaTest_VCols_vertData = "s1226:";
kha_Shaders.blender_Instancing_fragData = "s267:";
kha_Shaders.blender_Instancing_vertData = "s1352:";
...
kha_Shaders.blender_Instancing_NormalMapping_vertData = "s1695:";
...
kha_Shaders.blender_Instancing_NormalMapping_Texturing_vertData = "s1695:";
...
kha_Shaders.blender_Instancing_NormalMapping_Texturing_VCols_vertData = "s1755:";
...
kha_Shaders.blender_Instancing_NormalMapping_VCols_vertData = "s1755:";
...
kha_Shaders.blender_Instancing_Texturing_vertData = "s1319:";
...
kha_Shaders.blender_Instancing_Texturing_VCols_vertData = "s1379:";
...
kha_Shaders.blender_Instancing_VCols_vertData = "s1282:";
...
kha_Shaders.blender_NormalMapping_Texturing_vertData = "s1639:";
kha_Shaders.blender_NormalMapping_Texturing_VCols_fragData = "s4083:%";
kha_Shaders.blender_NormalMapping_Texturing_VCols_vertData = "s1699:";
...
kha_Shaders.blender_NormalMapping_VCols_vertData = "s1699:";
kha_Shaders.blender_Texturing_fragData = "s3935:";
kha_Shaders.blender_Texturing_vertData = "s1263:";
...
kha_Shaders.blender_Texturing_VCols_vertData = "s1323:";
...
kha_Shaders.blender_VCols_vertData = "s1226:";
...
kha_Shaders.final_fragData = "s216:dW5pZm9ybSBzYW1wbGVyMkQgc2hhZG93TWFwOwp2YXJ5aW5nIG1lZGl1bXAgdmVjMiB0ZXhDb29yZDsKdm9pZCBrb3JlKCl7CihnbF9GcmFnQ29sb3IgPSB2ZWM0KHRleHR1cmUyRChzaGFkb3dNYXAsIHRleENvb3JkKS54eXosIDEuMCkpOwp9CnZvaWQgbWFpbigpewprb3JlKCk7Cn0K";
kha_Shaders.final_vertData = "s256:";
