...
lue_node_CameraNode.prototype = $extend(lue_node_Node.prototype,{
	...
	,drawQuad: function(params,root,light) {
		var materialRes = lue_resource_Resource.getMaterial(params[0],params[1]);
...
lue_node_ModelNode.setMaterialConstants = function(g,context,materialContext) {
	...
	while(_g1 < _g) {
		...
		var c = context.resource.constants[i];
		lue_node_ModelNode.setMaterialConstant(g,context.constants[i],c,matc);
	}
...
lue_node_ModelNode.prototype = $extend(lue_node_Node.prototype,{
	...
	,render: function(g,context,camera,light,bindParams) {
		...
		while(_g11 < _g4) {
			...
			lue_node_ModelNode.setMaterialConstants(g,shaderContext,materialContexts[mi]);
...
var luegame_Interp = function() {
	hscript_Interp.call(this);
};
$hxClasses["luegame.Interp"] = luegame_Interp;
luegame_Interp.__name__ = ["luegame","Interp"];
luegame_Interp.__super__ = hscript_Interp;
luegame_Interp.prototype = $extend(hscript_Interp.prototype,{
	get: function(o,f) {
	...
	,__class__: luegame_Interp
});
var luegame_Script = function(script) {
	this.script = script;
	var parser = new hscript_Parser();
	this.expr = parser.parseString(script);
	this.interp = new luegame_Interp();
	this.interp.variables.set("App",lue_App);
	this.interp.variables.set("Math",Math);
	this.interp.variables.set("Std",Std);
	this.interp.variables.set("Scheduler",kha_Scheduler);
	this.interp.variables.set("Sys",kha_Sys);
	this.interp.execute(this.expr);
};
$hxClasses["luegame.Script"] = luegame_Script;
luegame_Script.__name__ = ["luegame","Script"];
luegame_Script.prototype = {
